...
> **Status: 154 probes · 11 compliance frameworks · SARIF + CBOM + PDF + XLSX exports · light/dark web UI — see [docs/STATUS.md](docs/STATUS.md).**
...
flowchart LR
    ...
    subgraph proc["pqcscan single process"]
        direction TB
        REG["probe registry<br/>(default_registry, 154 probes)"]
...
154 probes registered across 15 families. Each probe is a small `Probe` subclass that declares an `id`, a `family`, and `framework_tags` for the compliance engine to map findings.

```mermaid
flowchart TB
    REG["default_registry()<br/><b>154 probes</b>"]
...
flowchart LR
    ...
    PROBES["/probes<br/>154 probes by family"]
